...
namespace smartpurse
{
    public partial class ScanQrCode : PhoneApplicationPage
    {
        ...
        private void _phoneCamera_AutoFocusCompleted(object sender, CameraOperationCompletedEventArgs e)
        {
        ...
        void _bcReader_ResultFound(Result obj)
        {
            ...
            string s = obj.Text;
            if (s.Substring(0, 2) == "SP")
            {
                NavigationService.Navigate(new Uri("/Transaction.xaml?key=" + s, UriKind.Relative));
            }
            else
            {
                MessageBox.Show("Invalid Code, please scan again");
            }
